...
def unet_model(input_shape=(128, 128, 3), min_filter=16, kernel_size=(3, 3), up_down_size=(2, 2), strides=(2, 2), activation='elu'):
    '''
    ...
    # first encoder
    first_encoder = _encoder_block(input_l, filter_num[0], strides=strides, kernel_size=kernel_size, dropout=0.1, name='encoder_block_1', activation=activation)
    ...
    for i, filter_num in enumerate(decoder_filter_nums):
        ...
        x = _decoder_block(x, encoders[-(i+2)], decoder_filter_num[i], name='decoder_block_'+str(i+1), strides=strides, kernel_size=kernel_size, dropout=0.1, activation=activation)
...
def _encoder_block(x, filter_num, name='encoder_block', strides=(1, 1), kernel_size=(3, 3), dropout=0.1, activation='elu', down_size=(2, 2)):
    '''
    U-net encoder cnn block
    Conv -> activation -> dropout -> conv -> activation
    '''

    x = Conv2D(filter_num, kernel_size, strides=strides, name=name, activation=activation, padding='same')(x)
    x = Dropout(dropout)(x)
    x = Conv2D(filter_num, kernel_size, strides=strides, name=name, activation=activation, padding='same')(x)
...
def _decoder_block(x, skip_connect, filter_num, name='decoder_block', strides=(1, 1), kernel_size=(3, 3), dropout=0.1, activation='elu', up_size=(2, 2)):
    '''
    U-net decoder cnn block
    transpose conv -> concat skip_connect -> conv -> dropout -> conv
    '''

    x = Conv2DTranspose(filter_num, up_size, strides=strides, padding='same', activation=activation, name=name)(x)
    x = Concatenate()([x, skip_connect])
    x = Conv2D(filter_num, kernel_size, strides=strides, padding='same', activation=activation)(x)
    x = Dropout(dropout)(x)
    x = Conv2D(filter_num, kernel_size, strides=strides, padding='same', activation=activation)(x)
